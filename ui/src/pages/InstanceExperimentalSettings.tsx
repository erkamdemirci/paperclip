...
export function InstanceExperimentalSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.experimental.updateFailed"));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {experimentalQuery.error instanceof Error
          ...
          : t("settings.experimental.failed")}
      </div>
    );
  }
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("settings.experimental.isolatedWorkspaces")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("settings.experimental.isolatedWorkspacesDesc")}
            </p>
          </div>
          <button
            type="button"
